...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if ([HKHealthStore isHealthDataAvailable]) {
        ...
        [self.healthStore requestAuthorizationToShareTypes:writeDataTypes readTypes:readDataTypes completion:^(BOOL success, NSError *error) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [self updateUserBloodGlucose];
            });
        }];
    }
    
    //NOW It's only an local notification, but if we have server, we can set it to push notification which server can give a message to ask for data.
    NSLog(@"asdasda");
    ...
    localNotif.alertBody = [NSString stringWithFormat:@"Please enter your glucose level"];
    localNotif.alertAction = NSLocalizedString(@"Enter Glucose Level", nil);
...
- (NSSet *)dataTypesToRead {
    ...
    HKQuantityType *bloodGluocoseType = [HKObjectType quantityTypeForIdentifier:HKQuantityTypeIdentifierBloodGlucose];
    
    return [NSSet setWithObjects:dietaryCalorieEnergyType, activeEnergyBurnType, heightType, weightType, birthdayType, biologicalSexType, bloodGluocoseType, nil];
}

#pragma mark - Reading HealthKit Data

- (void)updateUserBloodGlucose {
    HKQuantityType *bloodGlucose = [HKObjectType quantityTypeForIdentifier:HKQuantityTypeIdentifierBloodGlucose];
    [self.healthStore aapl_mostRecentQuantitySampleOfType:bloodGlucose predicate:nil completion:^(HKQuantity *mostRecentQuantity, NSError *error) {
        if (!mostRecentQuantity) {
            ...
            // Determine the Glucose Level in Blood in the required unit.
            HKUnit *BloodSugerUnit = [[HKUnit gramUnitWithMetricPrefix:'m'] unitDividedByUnit:[HKUnit literUnitWithMetricPrefix:'d']];
            double usersGlucoseLevel = [mostRecentQuantity doubleValueForUnit:BloodSugerUnit];
            
            // Update the user interface.
            dispatch_async(dispatch_get_main_queue(), ^{
                self.heightValueLabel.text = [NSNumberFormatter localizedStringFromNumber:@(usersGlucoseLevel) numberStyle:NSNumberFormatterNoStyle];
            });
